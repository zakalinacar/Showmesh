 ...
 *
 * $Id: gl2ps.c,v 1.2 2003/02/01 14:52:56 canacar Exp $
 *
...
void gl2psPrintPostScriptHeader(void){
  ...
  if(gl2ps->options & GL2PS_DRAW_BACKGROUND){
    ...
    fprintf(gl2ps->stream,
	    ...
	    "closepath fill\n",
	    rgba[0], rgba[1], rgba[2], gl2ps->options & GL2PS_GRAY ? 'G' : 'C',
...
void gl2psPrintTeXHeader(void){
  ...
  if(gl2ps->filename && strlen(gl2ps->filename) < 256){
    ...
    if(i <= 0) strlcpy(name, gl2ps->filename, sizeof(name));
  }
  else
    strlcpy(name, "untitled", sizeof(name));
...
GL2PSDLL_API void gl2psText(const char *str, const char *fontname, GLint fontsize){
  ...
  strlcpy(prim->text->str, str, sizeof(GL2PSstring)); 
  prim->text->fontname = strdup(fontname);
